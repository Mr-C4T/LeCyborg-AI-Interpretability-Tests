...
Test cmd: 
python -m examples.visualise_original_data_attention --dataset-repo-id MrC4t/LeCyborg-so100_emg_sensor --policy-path MrC4t/LeCyborg_act --output-dir ./output/attention_analysis_results --device cpu --episode-id 1
...
def main():
    ...
    # --- DEBUG: full traceback on dataset load failure ---
    try:
        dataset = LeRobotDataset(args.dataset_repo_id)
        
        print(f"Dataset loaded successfully. Total episodes: {dataset.num_episodes}")
        if dataset.meta:
            # Print the attribute names of the meta object
            print(f"Dataset meta attributes: {dir(dataset.meta)}")
            # Or print the __dict__ content if available
            if hasattr(dataset.meta, '__dict__'):
                print(f"Dataset meta content: {dataset.meta.__dict__}")
            else:
                print(f"Dataset meta content: {dataset.meta}")
        else:
            print("No meta found")
                # Add debug to peek inside dataset
        try:
            sample = dataset[0]
            print(f"Sample data keys: {list(sample.keys())}")
            if 'episode_index' in sample:
                print(f"Sample episode_index: {sample['episode_index']}")
            else:
                print("Warning: 'episode_index' key not found in sample")
        except Exception as e:
    ...
    # --- DEBUG: full traceback on policy load failure ---
    try:
